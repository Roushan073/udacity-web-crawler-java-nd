...
final class WordCounts {
  ...
  static Map<String, Integer> sort(Map<String, Integer> wordCounts, int popularWordCount) {
    ...
    Stream<Map.Entry<String, Integer>> sortedWordCountStream = wordCounts.entrySet().stream()
            .sorted(new WordCountComparator()).limit(popularWordCount);

    // Converting to LinkedHashMap to maintain the order
    return sortedWordCountStream.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (key, value) -> key, LinkedHashMap::new));
  }
